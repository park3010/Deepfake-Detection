...
class VideoFrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 🔸 FFT / DCT / Wavelet 모드인 경우: 기존 BGR 경로 유지
        if self.use_fft or self.use_dct or self.use_wavelet:
            img_resized = self.resize(img)
            arr  = np.array(img_resized)[:, :, ::-1].astype(np.float32)  # BGR
        ...
        # 🔹 순수 RGB 모드: train과 동일 전처리
        x = self.rgb_transform(img)
        return x
    ...
    for root in roots:
        ...
        for vid in tqdm(vids, desc=f"[{label_value}] {os.path.basename(root)}"):
            ...
            for batch in tqdm(loader, desc=f" frames of {vid}", leave=False):
                ...
                # ✅ 추론은 inference_mode/no_grad로 막기 (그래프 생성 방지)
                with torch.inference_mode():
                    with autocast(enabled=use_amp):
                        logits = model(batch)
                        p = torch.softmax(logits, dim=1)[:, 1]  # (B,)
...
def main():
    ...
    parser.add_argument('--model',
        choices=['xception','resnet50','maxvit','hornet','coatnet','convnext','replknet31b', 'efficientnet-b7', 'efficientnet-b4', 'mlp'],
        required=True)
    ...
    if args.model == 'convnext':
        from Frequency_step2.models.convnextv2 import convnextv2_tiny
